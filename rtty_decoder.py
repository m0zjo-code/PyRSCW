...
def block_decode(samples):
    ...
    print(numpy.arange(SAMPLE_RATE/(2*oversample*RTTY_BAUDRATE), (SAMPLE_RATE*bits_per_frame)/RTTY_BAUDRATE, SAMPLE_RATE/(oversample*RTTY_BAUDRATE))).astype(int)
   
    sample_offsets = (numpy.arange(SAMPLE_RATE/(2*oversample*RTTY_BAUDRATE), (SAMPLE_RATE*bits_per_frame)/RTTY_BAUDRATE, SAMPLE_RATE/(oversample*RTTY_BAUDRATE))).astype(int)
    ...
    for j in range(0, len(samples)):
        ...
        sample_number += 1
        counter += 1
        #print(int(samples[sample_number]), int(samples[sample_number+1]), sample_number)
        if int(samples[sample_number]) == 1 and int(samples[sample_number+1]) == 0:
            # Sample bits at sample offsets
            bit_samples = numpy.array(samples)[sample_number+sample_offsets]
            ...
            data = numpy.array([int(d[0]) for d in data_samples])
            #data = numpy.flip(data, 0)
            
            data = 1-data
            
            
            val = data.dot(2**numpy.arange(data.size)[::-1])
            
            print "%d: Found frame: %i %s" % (offset, val, chr(val))
